...
.PHONY: help test lint claude-analyze claude-validate-schemas claude-loop

help:
	@printf "Targets:\n"
	@printf "  make test                     Run the Bats test suite\n"
	@printf "  make lint                     Run the local lint suite used in CI\n"
	@printf "  make claude-analyze           Run the analyze-config skill in Claude\n"
	@printf "  make claude-validate-schemas  Run the validate-schemas skill in Claude\n"
	@printf "  make claude-loop              Run validate-schemas then analyze-config in Claude\n"
	@printf "\nVariables:\n"
	@printf "  SESSIONS=%s\n" "$(SESSIONS)"
	@printf "  CLAUDE=%s\n" "$(CLAUDE)"

test:
	$(BATS) --print-output-on-failure tests/lib/metrics.bats tests/hooks/security.bats
...
lint:
	...
	$(SHELLCHECK) -x -P SCRIPTDIR hooks/*.sh hooks/lib/*.sh scripts/*.sh tests/setup_suite.bash
	$(SHFMT) -d hooks/*.sh hooks/lib/*.sh scripts/*.sh tests/setup_suite.bash
