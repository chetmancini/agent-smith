...
@test "run-agent-skill dispatches Gemini via non-interactive prompt mode" {
	local fakebin
	fakebin="$TEST_TMPDIR/fakebin"
	create_fake_agent "$fakebin" fake-gemini

	run env PATH="$fakebin:$PATH" AGENT_CLI=fake-gemini SESSIONS=40 \
		bash "$PROJECT_ROOT/scripts/run-agent-skill.sh" analyze-config --tool gemini
