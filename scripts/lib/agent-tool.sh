...
agent_smith_validate_tool_name() {
	case "$1" in
	claude | gemini | codex | opencode | pi) return 0 ;;
	*) return 1 ;;
	esac
}
...
agent_smith_detect_tool() {
	...
	if [ -n "$explicit_tool" ]; then
		...
		echo "Error: unsupported tool '$explicit_tool' (expected claude, gemini, codex, opencode, or pi)" >&2
		return 1
	fi
	...
	if agent_smith_tool_has_config pi; then
		pi_has_config=1
		config_count=$((config_count + 1))
	fi
	...
	if [ "$cli_count" -eq 1 ]; then
		...
		if [ "$pi_has_cli" -eq 1 ]; then
			printf 'pi\n'
			return 0
		fi
	fi

	echo "Error: unable to infer which agent to inspect. Set AGENT_SMITH_TOOL=claude, AGENT_SMITH_TOOL=gemini, AGENT_SMITH_TOOL=codex, AGENT_SMITH_TOOL=opencode, or AGENT_SMITH_TOOL=pi." >&2
	return 1
}
...
agent_smith_schema_url() {
	...
	pi) printf '%s\n' 'bundled://schemas/pi-settings.schema.json' ;;
	*)
...
agent_smith_schema_cache_path() {
	...
	pi) printf '%s\n' "${HOME}/.config/agent-smith/schemas/pi-settings.schema.json" ;;
	*)
		echo "Error: unsupported tool '$1'" >&2
		return 1
		;;
...
agent_smith_schema_metadata_path() {
	...
	pi) printf '%s\n' "${HOME}/.config/agent-smith/schemas/pi-settings.schema.metadata.json" ;;
	*)
		echo "Error: unsupported tool '$1'" >&2
		return 1
		;;
