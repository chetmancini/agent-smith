...
2. **Resolve the initiating agent**: use `claude` when running inside Claude Code, `codex` when running inside Codex, and `opencode` when running inside OpenCode. Do not mix them unless the user explicitly asks for a cross-agent plan
