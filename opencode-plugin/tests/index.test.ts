...
import { resetMetricsState } from "../src/lib/metrics.js"
...
describe("AgentSmithPlugin", () => {
  ...
  test("tracks permission replies using permissionID and response", async () => {
    ...
    await hooks["permission.ask"]?.({ id: "perm-1", type: "edit" }, { status: "ask" })
    await hooks.event?.({
      event: {
        type: "permission.replied",
        properties: { permissionID: "perm-1", response: "always" },
      },
    })
...
`
  ...
  test("tracks rejected permission replies as denied", async () => {
    ...
    await hooks["permission.ask"]?.({ id: "perm-2", type: "bash" }, { status: "ask" })
    await hooks.event?.({
      event: {
        type: "permission.replied",
        properties: { permissionID: "perm-2", response: "reject" },
      },
    })
